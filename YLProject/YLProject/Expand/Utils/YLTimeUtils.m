...
//  Doctor
//
//  Created by Conner on 2017/5/15.
//
...
+ (long long)getSystemTimeInterval {
    YLUserDefaults *defaults = [YLUserDefaults standardUserDefaults];
    long long realLocalTime = 0;
    long long serverTime = defaults.serverTime;
    long long lastSyncLocalRunTime = defaults.lastSyncLocalRunTime;
...
+ (void)syncServerTimeAndSystemRuntimeWith:(NSDate *)serverTime {
    if (serverTime != nil) {
        ...
        YLUserDefaults *ud = [YLUserDefaults standardUserDefaults];
        ud.serverTime = serverTimeLong;
        ud.lastSyncLocalRunTime = [self systemUptimeSinceLastBoot] * 1000;
        [ud synchronize];
    }
}
